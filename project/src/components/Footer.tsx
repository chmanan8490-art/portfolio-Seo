      ...
      <div className="container-px">
        <div className="grid md:grid-cols-4 gap-8 mb-10">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {navLinks.slice(0, 6).map((l) => (
                <li key={l.href}>
                  <a href={l.href} className="text-sm text-gray-400 hover:text-primary-400 transition-colors">{l.label}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {services.slice(0, 6).map((s) => (
                <li key={s.title}>
                  <a href="#services" className="text-sm text-gray-400 hover:text-primary-400 transition-colors">{s.title}</a>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mb-3">Weekly SEO tips delivered to your inbox.</p>
            <form onSubmit={(e) => e.preventDefault()} className="flex gap-2">
              <input
                type="email"
                placeholder="your@email.com"
                className="flex-1 rounded-lg bg-gray-800 border border-gray-700 px-3 py-2 text-sm text-white placeholder-gray-500 focus:border-primary-500 outline-none"
              />
        ...
        <div className="border-t border-gray-800 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-sm">
            ...
            <button onClick={onBackToTop} className="grid place-items-center w-9 h-9 rounded-full bg-gray-800 hover:bg-primary-600 transition-colors" aria-label="Back to top">
              <ArrowUp size={16} />
            </button>
          </div>
        </div>
